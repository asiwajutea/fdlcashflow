...
const CMSInnovations = () => {
  ...
  const fetchData = async () => {
    const { data } = await db.from('innovations').select('*').order('display_order');
    setItems(data || []);
  };
  useEffect(() => { fetchData(); }, []);
  ...
  const handleSave = async () => {
    ...
    if (editing.id) {
      const { error } = await db.from('innovations').update(editing).eq('id', editing.id);
      if (error) { toast.error(error.message); return; }
    } else {
      const { error } = await db.from('innovations').insert(editing);
      if (error) { toast.error(error.message); return; }
    }
  ...
  const handleDelete = async (id: string) => {
    ...
    await db.from('innovations').delete().eq('id', id);
    toast.success('Deleted'); fetchData();
  };

  return (
    <DashboardLayout title="Manage Innovations">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Link to="/cms"><Button variant="ghost" size="icon"><ArrowLeft className="h-4 w-4" /></Button></Link>
          <h2 className="text-xl font-bold text-foreground">Innovations</h2>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button onClick={() => setEditing({ ...empty })}><Plus className="h-4 w-4 mr-2" /> Add Innovation</Button></DialogTrigger>
          <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
            <DialogHeader><DialogTitle>{editing?.id ? 'Edit' : 'Add'} Innovation</DialogTitle></DialogHeader>
            {editing && (
